...
/*   Updated: 2024/06/10 16:16:48 by vtrevisa         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		Bureaucrat::executeForm(const AForm& f)
{
	...
	{
		f.execute(*this);
		std::cout << f.getName() << " executed by " << _name << std::endl;
	}
	catch (std::exception& e)
	{
		std::cout	<< f.getName() << " coundn't be executed by "
					<< _name << "because of: " << e.what()
					<<std::endl;
	}
}
